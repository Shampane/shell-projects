...
ping-host() {
    ...
    if [[ $is_single == true ]]; then
        log-time-event "[action]: start of ping $host" "$log"
    fi

    local average_time
    average_time=$(ping -c 2 -W 2 "$host" | grep "avg" | tr '/' ' ' | awk '{print $8}')
    ...
    log-message "$host / ${average_time} / $ipv4 / $ipv6" "$log"

    if [[ $is_single == true ]]; then
        log-time-event "[action]: ping $host completed sucessfully" "$log"
    fi
}
